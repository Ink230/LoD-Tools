...
@Component({
  ...
  template: `@if (editor.toolsOpen) {
    <div class="tools" [style.left]="'clamp(0px, ' + x + 'px, calc(100% - 52px))'" [style.top]="'clamp(0px, ' + y + 'px, calc(100% - 315px))'" (pointerdown)="$event.stopPropagation()">
...
export class WorldMapToolsComponent {
  ...
  move(event: PointerEvent) {
    ...
    this.y = Math.max(0, Math.min(this.drag.height - 315, this.drag.top + event.clientY - this.drag.y));
  }
  togglePinned() {
